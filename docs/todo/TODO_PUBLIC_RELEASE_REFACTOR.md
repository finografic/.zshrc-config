...
- [x] **Make the splash opt-out.** — Done. Measured at ~475 ms, not 300: `show-os-version-and-sys-info` 181 ms (`node`/`pnpm --version`), `show-ports` 76 ms (`lsof`), `show-tmutil-snapshots` 73 ms, `show-splash-neofetch` 48 ms, `show-custom-launch-agents` 36 ms. `ZSHRC_SPLASH` is `1` always / `0` never / unset = outermost interactive shell only. **Deviation from the sketch:** the default checks `-o login` _before_ falling back to `SHLVL`, rather than `[[ -o login ]]` alone — a terminal emulator always starts a login shell, so a real window keeps its splash even though `SHLVL` is inherited and can be >1 for unrelated reasons. Nested shells: **477 ms → 4.4 ms**; a real terminal window is deliberately unchanged. `bench-startup.zsh` now forces `ZSHRC_SPLASH=1`, or the benchmark would have quietly stopped measuring ~475 ms that a real terminal still pays.
...
- [x] Re-run the benchmark; update `docs/benchmarks/`. — Cumulative table recorded in [`docs/benchmarks/README.md`](../benchmarks/README.md). Sandbox, n=10, splash forced on: `home-macos` −69%, `vscode` −67%, `office-macos` −66%, `home-linux`/`server-linux` −63%, `android` −77%. `codex` and `docker-dev` moved only ~6%, which is the expected result rather than a disappointment — both already skipped the plugin bundle via their bootstrap early exits, and that is precisely where the nvm duplication lived. **Still needs a real-machine re-run to refresh `baseline.json`.**
- [ ] **Remaining, not yet done:** `autoload` the cold functions, `zcompile` the hot path, and the rest of the shell-out audit (`sw_vers`, `uname`, `hostname`, `brew shellenv` caching). After the three wins above, the profiles sit at ~1.2–1.9 s in the sandbox, still well over the 400 ms/150 ms budget. The largest single remaining item is `source nvm.sh` itself at ~449 ms + 58 ms of `bash_completion`, which needs genuine deferred loading — it conflicts with running `load-nvmrc` at startup, so it is a design change rather than a tweak.
...
- [ ] `git mv _zenvs profiles` (keeps history for all 8 profile directories).
- [ ] Update path strings — `$ZENV_PATH` is built in exactly two places (`main.zsh`, and each profile's own header), plus `core/profile.zsh`'s feature resolution and `lib/widgets.zsh`'s banner lookup. Keep `$ZENV` as the variable name (D2's own recommendation) so the churn stays limited to literal `_zenvs` strings.
- [ ] Update `tests/test-profile-boot.zsh`, `tests/test-profile-loader.zsh`, `scripts/bench-startup.zsh`, `README.md`, `AGENTS.md`, `.agents/handoff.md`, and this doc.
- [ ] **No compat shim.** The original bullet offered one "if you want to be gentle to your other machines" — unnecessary here: `$ZSHRC_ROOT` is a git checkout, so any other machine gets the rename atomically on its next `git pull`, and a stale shim would just be one more thing to remove later. The one real risk is a machine mid-`zupdate` during the rename, which re-cloning or a plain `git pull` resolves.
- [ ] Grep sweep for stragglers: `grep -rn "_zenvs" --include="*.zsh" --include="*.md" --include="*.yml" .` must return nothing but history notes in `docs/todo/` and `.agents/`.
