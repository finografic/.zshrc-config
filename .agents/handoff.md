...
**P4.3 done out of order** (Sonnet-tier), ahead of Phase 3, for the reason flagged above: `scripts/bench-startup.zsh` (+ `--all-profiles`/`--json`/`--save`), one-command `zprof` via `ZSHRC_PROFILE=1`, a soft-fail CI ratio check, and `docs/benchmarks/baseline.json` + `README.md`. **The baseline has a load-bearing caveat: it was captured inside this AI agent's sandbox, not a real machine**, and shows signs of startup-specific resource contention (a `gitstatusd` init failure; `node --version` costing 1.5s of `zprof` self-time vs 17ms standalone in the same sandbox) — absolute numbers are not trustworthy against the 400ms/150ms budget until re-run on a real machine (`scripts/bench-startup.zsh --all-profiles -n 20 --save`). What the same-environment _ratios_ show reliably: `codex` (~1.3s) ≪ `docker-dev` (~1.7s) < `vscode` (~3.4s) ≪ full profiles (~4.3–5.6s) — the bootstrap early-exit architecture working as designed, and a real finding that `vscode`'s "minimal" profile still pays full `antidote`/`compinit`/prompt cost in `bootstrap/`, unlike `codex` (candidate for P4.4). Two bugs found building the tool: `ZENV_FORCE` alone doesn't reach vscode/codex's fast paths (main.zsh's early exits test real env signals, not `$ZENV`); a bare `local name` redeclaration inside a `{ } > file` block triggers zsh's `typeset` display-mode and corrupted the first `--save` JSON output.
