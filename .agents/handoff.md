...
**P6.1 done**: `update-config.zsh` (`zupdate`) rewritten. Stages tracked changes only (`git add -u`), listing untracked files with their sizes rather than sweeping them in — the old `git add .` is how 66 MB of binaries got committed. Every message path produces a commitlint-valid subject, so the hook stops being bypassed; `--sync` is the only auto-message. Rebase conflicts print recovery steps; it refuses to start mid-rebase. A secret scan runs before every push — `zconf scan` where Node exists, a dependency-free `git grep` where it does not — skippable only via an explicit, warned `--no-scan`. Stays pure zsh. `tests/test-zupdate.zsh` (23 cases) runs against a throwaway repo with a local remote, so it never touches this one; it is in CI.
