...
- [ ] **Grep sweep — partially clear.** IPv4 literals: **zero** remain in tracked non-doc files. Emails, prior-employer handles, `hostinger`, `justin.rankin`: **zero**. Still outstanding, each owned by a later phase and each an _organisational name_ rather than a secret:
