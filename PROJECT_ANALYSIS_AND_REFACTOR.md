...
4. **`office-macos` mutates global git config on every shell.** It runs
   `git config --global user.email "REDACTED-EMAIL"` and copies
...
- **`git config --global user.email "REDACTED-EMAIL"`** and the
...
- `lib/` is the healthy core; leave it. Consider splitting the 340-line
  `lib/dev.zsh` by concern if it keeps growing.
